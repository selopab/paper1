...
*OLS (FS)
reg p_actor i.treatment time_instrument i.junta tipodeabogado trabajador_base fechaDemanda, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_MaxTreatment.xls", replace ctitle("OLS FS")

*Probit (FS)
probit p_actor i.treatment time_instrument i.junta tipodeabogado trabajador_base fechaDemanda, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_MaxTreatment.xls", append ctitle("Probit FS")
...
*CF
*Probit - Interaction
reg seconcilio i.treatment##i.p_actor i.junta gen_resid_pr tipodeabogado trabajador_base fechaDemanda, vce(bootstrap, reps(1000)) cluster(fecha)
qui su seconcilio if e(sample)
local DepVarMean=r(mean)
qui su p_actor if e(sample)
local IntMean=r(mean)
outreg2 using "$sharelatex/Tables/reg_results/CF_MaxTreatment.xls", append ctitle("CF Probit") addstat(DepVarMean, `DepVarMean', IntMean, `IntMean')
...
*Probit (FS)
probit p_actor i.treatment i.junta time_hr2-time_hr8 tipodeabogado trabajador_base fechaDemanda, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_MaxTreatment.xls", append ctitle("Probit FS")
...
*CF
*Probit - Interaction
reg seconcilio i.treatment##i.p_actor i.junta  gen_resid_pr8 tipodeabogado trabajador_base fechaDemanda, vce(bootstrap, reps(1000)) cluster(fecha)
qui su seconcilio if e(sample)
local DepVarMean=r(mean)
qui su p_actor if e(sample)
local IntMean=r(mean)
