...
keep if renglon==1
	...
	*Same day conciliation
	...
	outreg2 using "$sharelatex/Tables/reg_results/treatment_effectsITT_p2.xls", replace ctitle("Same day. P2")  ///
	...
	*Interaction employee was present
	...
	qui su seconcilio if e(sample)
	local DepVarMean=r(mean)
	outreg2 using  "$sharelatex/Tables/reg_results/treatment_effectsITT_p2.xls", append ctitle("Same day. P2")  ///
	...
	*Long run simple
	...
	qui su p_actor if e(sample)
	local IntMean=r(mean)
	outreg2 using  "$sharelatex/Tables/reg_results/treatment_effectsITT_p2.xls", append ctitle("Long Run")  ///
	addtext(Court Dummies, Yes, Casefile Controls, No) addstat(Dependent Variable Mean, `DepVarMean') ///
	keep(2.treatment 1.p_actor 2.treatment#1.p_actor )
