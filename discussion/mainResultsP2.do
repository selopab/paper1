...
keep if renglon==1
	...
	*Same day conciliation
	reg seconcilio i.treatment i.junta if treatment!=0, robust  cluster(fecha)
	qui su seconcilio if e(sample)
	local DepVarMean=r(mean)
	outreg2 using  "$sharelatex/Tables/reg_results/treatment_effectsITT_phase2.xls", replace ctitle("Same day. P2")  ///
	...
	*Interaction employee was present
	...
	qui su seconcilio if e(sample)
	local DepVarMean=r(mean)
	outreg2 using  "$sharelatex/Tables/reg_results/treatment_effectsITT_phase2.xls", append ctitle("Same day. P2")  ///
	addtext(Court Dummies, Yes, Casefile Controls, No) addstat(Dependent Variable Mean, `DepVarMean', Interaction Mean,`IntMean') ///
	keep(2.treatment 1.p_actor 2.treatment#1.p_actor )	

	*Long run
	reg convenio_m5m i.treatment i.junta if treatment!=0, robust  cluster(fecha)
	qui su convenio_m5m if e(sample)
	local DepVarMean=r(mean)
	qui su p_actor if e(sample)
	local IntMean=r(mean)
	outreg2 using  "$sharelatex/Tables/reg_results/treatment_effectsITT_phase2.xls", append ctitle("Long Run. Pooled")  ///
	addtext(Court Dummies, Yes, Casefile Controls, No) addstat(Dependent Variable Mean, `DepVarMean', Interaction Mean,`IntMean') ///
	keep(2.treatment 1.p_actor 2.treatment#1.p_actor )	
