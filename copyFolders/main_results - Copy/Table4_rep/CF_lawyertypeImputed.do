...
gen tipodeabogadoImputed = abogado_pub
replace tipodeabogadoImputed = 0 if missing(tipodeabogadoImputed)
gen missingCasefiles = missing(abogado_pub)
...
*OLS (FS)
reg p_actor i.treatment time_instrument i.junta missingCasefiles#(`imputedControls') missingCasefiles, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyerImputed.xls", replace ctitle("OLS FS")

*Probit (FS)
probit p_actor i.treatment time_instrument i.junta missingCasefiles#(`imputedControls') missingCasefile, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyerImputed.xls", append ctitle("Probit FS")
...
*CF
*Probit - Interaction
reg seconcilio i.treatment##i.p_actor i.junta gen_resid_pr missingCasefiles#(`imputedControls') missingCasefiles, vce(bootstrap, reps(1000)) cluster(fecha)
qui su seconcilio if e(sample)
local DepVarMean=r(mean)
qui su p_actor if e(sample)
local IntMean=r(mean)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyerImputed.xls", append ctitle("CF Probit") addstat(DepVarMean, `DepVarMean', IntMean, `IntMean')
...
*Probit (FS)
probit p_actor i.treatment i.junta time_hr2-time_hr8 missingCasefiles#(`imputedControls') missingCasefiles, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyerImputed.xls", append ctitle("Probit FS")
...
*CF
*Probit - Interaction
reg seconcilio i.treatment##i.p_actor i.junta  gen_resid_pr8 missingCasefiles#(`imputedControls') missingCasefiles, vce(bootstrap, reps(1000)) cluster(fecha)
qui su seconcilio if e(sample)
local DepVarMean=r(mean)
qui su p_actor if e(sample)
local IntMean=r(mean)
