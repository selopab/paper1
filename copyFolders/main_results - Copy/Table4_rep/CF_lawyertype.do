...
*OLS (FS)
reg p_actor i.treatment time_instrument i.junta `controls', r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyer.xls", replace ctitle("OLS FS") ///
 addtext(Court Dummies, Yes, Casefile Controls, Yes, Imputed Controls, No)

*Probit (FS)
probit p_actor i.treatment time_instrument i.junta, r cluster(fecha)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyer.xls", append ctitle("Probit FS") ///
 addtext(Court Dummies, Yes, Casefile Controls, Yes, Imputed Controls, No)
...
*CF
*Probit - Interaction
reg seconcilio i.treatment##i.p_actor i.junta gen_resid_pr `controls', vce(bootstrap, reps(1000)) cluster(fecha)
qui su seconcilio if e(sample)
local DepVarMean=r(mean)
qui su p_actor if e(sample)
local IntMean=r(mean)
outreg2 using "$sharelatex/Tables/reg_results/CF_ITT_onlyLawyer.xls", append ctitle("CF Probit") addstat(DepVarMean, `DepVarMean', IntMean, `IntMean') ///
 addtext(Court Dummies, Yes, Casefile Controls, Yes, Imputed Controls, No)
