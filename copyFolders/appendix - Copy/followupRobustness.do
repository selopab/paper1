...
local int = 2 + `i'*.3
gen npv_`i'=.
replace npv_`i'=(ganancia/(1+(`int')/100)^months)*(1-${perc_pag})-${pago_pri} if abogado_pub==0
replace npv_`i'=(ganancia/(1+(`int')/100)^months)-${pago_pub} if abogado_pub==1 
gen asinhNPV_`i' = asinh(npv_`i')
}
...
local int = 2 + `i'*.3
gen npvImputed_`i'=.
replace npvImputed_`i'=(gananciaImputed/(1+(`int')/100)^months)*(1-${perc_pag})-${pago_pri} if abogado_pub==0
replace npvImputed_`i'=(gananciaImputed/(1+(`int')/100)^months)-${pago_pub} if abogado_pub==1 
gen asinhNPVImputed_`i' = asinh(npvImputed_`i')
...
**************
local r = 2 + `i'*.3
local anual_r = ((1+(`r')/100)^12 -1)*100 


*Primero sin controles

*4) Treatment presence interaction + casefile level controls
reg asinhNPV_`i' i.treatment i.p_actor i.treatment#i.p_actor `controls' , robust cluster(fecha)	
	qui su asinhNPV_`i' if e(sample) & p_actor == 1
	local IntMean=r(mean)
	qui su asinhNPV_`i' if e(sample)
	local DepVarMean=r(mean)
outreg2 using "$sharelatex\Tables\reg_results\followUpRobustness_normal.xls", replace ctitle("asinhNPV. Controls") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', Anual rate, `anual_r')  keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor)
...
*4) Treatment presence interaction + casefile level controls
reg asinhNPVImputed_`i' i.treatment i.p_actor i.treatment#i.p_actor `controls', robust cluster(fecha)	
	qui su asinhNPVImputed_`i' if e(sample) & p_actor ==1
	local IntMean=r(mean)
	qui su asinhNPVImputed_`i' if e(sample)
	local DepVarMean=r(mean)
outreg2 using "$sharelatex\Tables\reg_results\followUpRobustness_imputed.xls", replace ctitle("asinhNPVImputed. Controls") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', Anual rate, `anual_r') keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor)
...
reg asinhNPV_`i' i.treatment i.p_actor i.treatment#i.p_actor `controls' , robust cluster(fecha)	
	...
	qui su asinhNPV_`i' if e(sample)
	local DepVarMean=r(mean)
outreg2 using "$sharelatex\Tables\reg_results\followUpRobustness_normal.xls", append ctitle("asinhNPV. Controls") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', Anual rate, `anual_r')  keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor)
...
reg asinhNPVImputed_`i' i.treatment i.p_actor i.treatment#i.p_actor `controls', robust cluster(fecha)	
	...
	qui su asinhNPVImputed_`i' if e(sample)
	local DepVarMean=r(mean)
outreg2 using "$sharelatex\Tables\reg_results\followUpRobustness_imputed.xls", append ctitle("asinhNPVImputed. Controls") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', Anual rate, `anual_r') keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor)
}
